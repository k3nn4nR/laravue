        ...
        <table class="table" id="my-table-qr">
            ...
            <tbody>
                <tr v-for="person in peopleSelected" :key="person.id">
                    ...
                    <td>
                        <vue-qr :text="person.person_documents[person.person_documents.length-1].id_number" :qid="person.person_documents[person.person_documents.length-1].id_number"></vue-qr>
                    </td>
                </tr>
            </tbody>
        </table>
    ...
    export default {
        ...
        methods:{
            generatePDF() {
                ...
                for(let i = 0; i<this.personasSelected.length;i++)
                {
                    if(i%10==0 && i != 0)
                    {
                        doc.addPage()
                    ...
                    doc.setFont("arial")
                    doc.setFontSize(8)
                    doc.text(this.personasSelected[i].ape_paterno+' '+this.personasSelected[i].ape_materno+',', 19, index+10)
                    doc.text(this.personasSelected[i].nombre, 20, index+14)
                    doc.text(this.personasSelected[i].documentos[0].id_number, 72, index+12)
                    doc.addImage(this.personasSelected[i].qr_code, 'PNG', 90, index, 20, 20)
